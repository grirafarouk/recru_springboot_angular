import { utilisateursComponent } from './../../../views/Administration/Utilisateurs/utilisateurs.component';
...
@Component({
  selector: 'app-reportingtable',
  templateUrl: './reportingtable.component.html',
})
export class ReportingtableComponent implements OnInit {
  ...
  pertinenceSearch() {
    if (this.item2.pertinence != null) {
      this.items = this.items.filter(res => {
        return res.pertinence == this.item2.pertinence;
      });
  ...
  origineSearch() {
    if (this.item2.origine != null) {
      this.items = this.items.filter(res => {
        return res.origine.toLocaleLowerCase().match(this.item2.origine.toLocaleLowerCase());
      });
  ...
  searchlieuentretien() {
    if (this.item2.lieuEntretien != null) {
      this.items = this.items.filter(res => {
        return res.lieuEntretien === this.item2.lieuEntretien;
      });
      this.verifier_critere_de_recherche = true;
    }
    else {
  ...
  searchingdisponible() {
    ...
    if (this.item2.disponibilite != null) {
      this.items = this.items.filter(res => {
        return res.disponibilite === this.item2.disponibilite;
      });
      this.verifier_critere_de_recherche = true;
    }
    else {
  ...
  searchcharge(event) {
    ...
    if (this.item2.charge != null) {
      this.items = this.items.filter(res => {
        return res.charge.toLocaleLowerCase().match(this.item2.charge.toLocaleLowerCase());
      });
      this.verifier_critere_de_recherche = true;
      this.doRechercheCandidat();
  ...
  searchingdateDemarrageFormation(event) {
    this.item2.dateDemarrageFormation = event
    if (this.item2.dateDemarrageFormation != null) {
      this.items = this.items.filter(res => {
        return res.dateDemarrageFormation === this.item2.dateDemarrageFormation;
      });
      this.verifier_critere_de_recherche = true;
    }
    else {
